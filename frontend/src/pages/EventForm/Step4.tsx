...
const Step4: React.FC<Step4Props> = ({ formData, updateForm, nextStep, prevStep }) => {
  return (
      ...
      <div className="w-full max-w-2xl flex flex-row">
        ...
        <div className="flex-1 flex flex-col gap-6 justify-center p-8">
          <input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => updateForm({ budget: e.target.value })}
          />
          <input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => updateForm({ footfall: e.target.value })}
          />
        </div>
      </div>
        ...
        <div className="flex gap-8">
          {[1, 2, 3, 4, 5].map((n) => (
            <div key={n} className="flex flex-col items-center">
              <span className={n === 4 ? 'text-purple-400' : 'text-white'}>{n}.</span>
              <div className={`w-16 h-1 ${n === 4 ? 'bg-purple-400' : 'bg-white'}`}></div>
            </div>
          ))}
        </div>
      </div>
